...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        self.use_h5 = sampler_cfg.get('USE_H5', False)
        self.use_del = sampler_cfg.get('USE_DEL', True)

        for db_info_path in sampler_cfg.DB_INFO_PATH:
            db_info_path = self.root_path.resolve() / db_info_path
            if not db_info_path.exists():
                assert 0, f"DB info file not found: {self.root_path}"
    ...
    def __del__(self):
        if self.use_shared_memory and self.use_del:
            ...
            self.logger.info('GT database has not been removed from shared memory')
    ...
    def load_db_to_shared_memory(self):
        ...
        sa_key = self.sampler_cfg.DB_DATA_PATH[0]
        if self.sampler_cfg.get('APP_TAG', None) is not None:
            app_tag = self.sampler_cfg.APP_TAG
            sa_key = f"{app_tag}_{sa_key}"
        if cur_rank % num_gpus == 0 and not os.path.exists(f"/dev/shm/{sa_key}"):
            if self.use_h5:
                h5_dp_data_path = db_data_path.with_suffix('.h5')
                self.logger.info(f'Loading h5 data from {h5_dp_data_path}')
                import h5py
                with h5py.File(h5_dp_data_path, 'r') as f:
                    gt_database_data = f['data'][:]
            else:
    ...
    def load_db_to_shared_memory_img(self):
        ...
        if cur_rank % num_gpus == 0 and not os.path.exists(f"/dev/shm/{sa_key}"):
            if self.use_h5:
                import h5py
                h5_dp_data_path_img = db_data_path_img.with_suffix('.h5')
                self.logger.info(f'Loading h5 data from {h5_dp_data_path_img}')
                with h5py.File(h5_dp_data_path_img, 'r') as f:
                    gt_database_data = f['data'][:]
            else:
    ...
    def copy_paste_to_image_nuscenes(self, data_dict, crop_feat, gt_number, point_idxes=None):
        ...
        # if 'img_sweeps' in data_dict.keys():
        #     img_sweeps = []
        #     for img_sweep in data_dict['img_sweeps']:
        #         crop_imgs = []
        #         for img, img_process_info in zip(img_sweep, img_process_infos):
        #             resize,crop = img_process_info[:2]
        #             img = Image.fromarray(img)
        #             resize_dims = (int(W * resize), int(H * resize))
        #             img = img.resize(resize_dims)
        #             img = img.crop(crop)
        #             crop_imgs.append(img)
        #         img_sweeps.append(crop_imgs)
        #     data_dict['img_sweeps'] = img_sweeps
        return data_dict
    ...
    def sample_gt_boxes_2d_nuscenes(self, data_dict, sampled_boxes, valid_mask, sampled_dict):
        ...
        existed_boxes2d = torch.Tensor(data_dict['gt_boxes2d'])
        if existed_boxes2d.dim() == 1:
            existed_boxes2d = existed_boxes2d.unsqueeze(0) if existed_boxes2d.shape[0] > 0 else torch.zeros((0, 6))
    ...
    def copy_paste_to_image(self, img_aug_gt_dict, data_dict, points):
        if self.img_aug_type == 'kitti':
            ...
            gt_boxes2d = np.array(img_aug_gt_dict['gt_boxes2d'])
            crop_boxes2d = np.array(img_aug_gt_dict['crop_boxes2d'])
            data_dict['gt_boxes2d'] = np.concatenate(
                [gt_boxes2d.reshape(-1, 6) if gt_boxes2d.ndim == 1 else gt_boxes2d,
                 crop_boxes2d.reshape(-1, 6) if crop_boxes2d.ndim == 1 else crop_boxes2d], axis=0)
            ...
            gt_boxes2d = np.array(img_aug_gt_dict['gt_boxes2d'])
            crop_boxes2d = np.array(img_aug_gt_dict['crop_boxes2d'])
            # Pad 5-col arrays to 6 cols (insert dummy score=0 at position 4)
            def _pad_score(arr):
                if arr.ndim == 1 and len(arr) > 0 and arr.shape[0] == 5:
                    return np.insert(arr, 4, 0)
                elif arr.ndim == 2 and arr.shape[1] == 5:
                    return np.insert(arr, 4, 0, axis=1)
                return arr
            gt_boxes2d = _pad_score(gt_boxes2d)
            crop_boxes2d = _pad_score(crop_boxes2d)
            data_dict['gt_boxes2d'] = np.concatenate(
                [gt_boxes2d.reshape(-1, 6) if gt_boxes2d.ndim == 1 else gt_boxes2d,
                 crop_boxes2d.reshape(-1, 6) if crop_boxes2d.ndim == 1 else crop_boxes2d], axis=0)
