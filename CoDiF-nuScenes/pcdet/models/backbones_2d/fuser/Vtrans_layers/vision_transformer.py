...
class DinoVisionTransformer(nn.Module):
    ...
    def prepare_tokens_with_masks(self, x, masks=None):
        ...
        a=self.interpolate_pos_encoding(x, w, h)
    ...
    def get_intermediate_layers(
        ...
        if reshape:
            B, _, w, h = x.shape
            outputs = [
                out.reshape(B, w // self.patch_size[1], h // self.patch_size[0], -1).permute(0, 3, 1, 2).contiguous()
                for out in outputs
            ]
...
def vit_small(patch_size=16, num_register_tokens=0, in_chans=4,**kwargs):
