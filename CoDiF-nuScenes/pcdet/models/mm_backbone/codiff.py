...
from pcdet.models.backbones_3d.local_mamba import GlobalMamba #GlobalMamba==Voxelmamba
from functools import partial
from ...utils.spconv_utils import replace_feature, spconv

from pcdet.models.backbones_3d.lion_backbone_one_stride import LocalMamba  #LocalMamba==Lionlayer
...
class Codiff(nn.Module):
    def __init__(self, model_cfg, use_map=False, **kwargs):
        super().__init__()
        _mamba_cfg = model_cfg.get('MAMBA_CFG', {})
        self.depth_mm = _mamba_cfg.get('depth_mm', 2)
        self.image_shape = model_cfg.get('IMAGE_SHAPE', [256, 704])
        self.localmamba_info = {
            'NAME': 'Mamba',
            'CFG': {
                'd_state': _mamba_cfg.get('d_state', 16),
                'd_conv': _mamba_cfg.get('d_conv', 4),
                'expand': _mamba_cfg.get('expand', 2),
                'drop_path': _mamba_cfg.get('drop_path', 0.2)
            }
        }
        ...
        # depths = ([1])
        depths = (2,2) #(2,2) /(4,4)
        out_indices =[1] #[0,1]:75.27-72.94; [1]:75.27-72.95
        ...
        self.out_indices = self.model_cfg.out_indices #[0,1]
    ...
    def forward(self, batch_dict):
        ...
        for stage_id in range(self.stage_num):  # 1
            ...
            for i in range(len(block_layers)):  # 4 (1 2 1)
                ...
                if i >= self.lidar2image_start and i < self.lidar2image_end:  # ? i == 11
    ...
    def _input_preprocess2(self, batch_dict):
        ...
        imgs = imgs_list[1] if False else imgs_list[0] #false
         #6,128,64,176
