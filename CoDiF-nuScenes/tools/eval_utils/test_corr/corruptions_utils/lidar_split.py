...
def lidar_split(lidar, pts_pose):
    ...
    if not pose_matrix.shape[0] == 2:
        ...
        for t in range(num_step):
            ...
            orig_idx_list.append(np.where(scan_area)[0])
            ...
            '''
            Now calculate GPS compensation transformation (inverse of motion correction)
            '''
            trans_matrix_gps = np.linalg.inv(trans_matrix)
    ...
    # Concatenate original indices to track output->input mapping
    inv_perm = np.concatenate(orig_idx_list) if orig_idx_list else np.arange(start_pc.shape[1])
    return trans_matrix_gps_tensor, pc_timestap_list, inv_perm
