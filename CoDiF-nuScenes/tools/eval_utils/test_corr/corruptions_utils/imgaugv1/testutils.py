...
from augmentables.kps import KeypointsOnImage


class ArgCopyingMagicMock(mock.MagicMock):
    """A MagicMock that copies its call args/kwargs before storing the call.

    This is useful for imgaugv1 as many augmentation methods change data
    in-place.
...
def is_parameter_instance(param, param_type):
    ...
    is an instance of :class:`imgaugv1.parameters.AutoPrefetcher`, then it
    ...
    Parameters
    ----------
    param : imgaugv1.parameters.StochasticParameter
        The parameter to check.

    param_type : type
        The desired type. Similar as in ``isinstance``.
        E.g. ``imgaugv1.parameters.Deterministic``.
