"""Collection of basic functions used throughout imgaugv1."""
...
def warn_deprecated(msg, stacklevel=2):
    ...
    The used warning is ``imgaugv1.imgaugv1.DeprecationWarning``.
...
def seed(entropy=None, seedval=None):
    """Set the seed of imgaugv1's global RNG.

    The global RNG controls most of the "randomness" in imgaugv1.
    ...
    .. note::
        ...
        future. The preferred way to seed `imgaugv1` is via
        :func:`~imgaugv1.random.seed`.
...
@deprecated("imgaugv1.random.normalize_generator")
def normalize_random_state(random_state):
    ...
    random_state : None or int or numpy.random.Generator or numpy.random.BitGenerator or numpy.random.bit_generator.SeedSequence or numpy.random.RandomState
        See :func:`~imgaugv1.random.normalize_generator`.
    ...
    """
    import random
    return random.normalize_generator_(random_state)


@deprecated("imgaugv1.random.get_global_rng")
def current_random_state():
    """Get or create the current global RNG of imgaugv1.
    ...
    """
    import random
    return random.get_global_rng()


@deprecated("imgaugv1.random.convert_seed_to_rng")
def new_random_state(seed=None, fully_random=False):
    ...
    if seed is None:
        if fully_random:
            return random.RNG.create_fully_random()
        return random.RNG.create_pseudo_random_()
...
@deprecated("imgaugv1.random.copy_generator_unless_global_rng")
def copy_random_state(random_state, force_copy=False):
    ...
    import random
    if force_copy:
        return random.copy_generator(random_state)
    return random.copy_generator_unless_global_generator(random_state)


@deprecated("imgaugv1.random.derive_generator_")
def derive_random_state(random_state):
    ...
    """
    import random
    return random.derive_generator_(random_state)


@deprecated("imgaugv1.random.derive_generators_")
def derive_random_states(random_state, n=1):
    ...
    """
    import random
    return random.derive_generators_(random_state, n=n)


@deprecated("imgaugv1.random.advance_generator_")
def forward_random_state(random_state):
    ...
    """
    import random
    random.advance_generator_(random_state)
...
def pool(arr, block_size, func, pad_mode="constant", pad_cval=0,
    ...
    **Supported dtypes**:
    ...
    pad_mode : str, optional
        ...
        without remainder. See :func:`~imgaugv1.imgaugv1.pad` for details.
...
def avg_pool(arr, block_size, pad_mode="reflect", pad_cval=128,
    ...
    **Supported dtypes**:
        ...
        See :func:`~imgaugv1.imgaugv1.pool`.

    Parameters
    ----------
    arr : (H,W) ndarray or (H,W,C) ndarray
        Image-like array to pool.
        See :func:`~imgaugv1.imgaugv1.pool` for details.
    ...
    pad_mode : str, optional
        Padding mode to use if the array cannot be divided by `block_size`
        without remainder.
        See :func:`~imgaugv1.imgaugv1.pad` for details.
...
def _avg_pool_uint8(arr, block_size, pad_mode="reflect", pad_cval=128):
    from augmenters.size import pad_to_multiples_of
...
def show_grid(images, rows=None, cols=None):
    ...
    This will show a window of the results of :func:`~imgaugv1.imgaugv1.draw_grid`.

    **Supported dtypes**:

        minimum of (
            :func:`~imgaugv1.imgaugv1.draw_grid`,
            :func:`~imgaugv1.imgaugv1.imshow`
        )

    Parameters
    ----------
    images : (N,H,W,3) ndarray or iterable of (H,W,3) array
        See :func:`~imgaugv1.imgaugv1.draw_grid`.
...
def imshow(image, backend=IMSHOW_BACKEND_DEFAULT):
    ...
    **Supported dtypes**:
    ...
    if backend == "cv2":
        ...
        win_name = "imgaugv1-default-window"
        ...
        fig.canvas.set_window_title("imgaugv1.imshow(%s)" % (image.shape,))
...
def apply_lut(image, table):
    ...
    **Supported dtypes**:
        ...
        See :func:`~imgaugv1.imgaugv1.apply_lut_`.
...
def _mark_moved_class_or_function(class_name_old, module_name_new,
    ...
    def _func(*args, **kwargs):
        import importlib
        warn_deprecated(
            "Using imgaugv1.imgaugv1.%s is deprecated. Use %s.%s instead." % (
...
MOVED = [
    ("Keypoint", "imgaugv1.augmentables.kps", None),
    ("KeypointsOnImage", "imgaugv1.augmentables.kps", None),
    ("BoundingBox", "imgaugv1.augmentables.bbs", None),
    ("BoundingBoxesOnImage", "imgaugv1.augmentables.bbs", None),
    ("Polygon", "imgaugv1.augmentables.polys", None),
    ("PolygonsOnImage", "imgaugv1.augmentables.polys", None),
    ("MultiPolygon", "imgaugv1.augmentables.polys", None),
    ("_ConcavePolygonRecoverer", "imgaugv1.augmentables.polys", None),
    ("HeatmapsOnImage", "imgaugv1.augmentables.heatmaps", None),
    ("SegmentationMapsOnImage", "imgaugv1.augmentables.segmaps", None),
    ("Batch", "imgaugv1.augmentables.batches", None),
    ("BatchLoader", "imgaugv1.multicore", None),
    ("BackgroundAugmenter", "imgaugv1.multicore", None),
    ("compute_geometric_median", "imgaugv1.augmentables.kps", None),
    ("_convert_points_to_shapely_line_string", "imgaugv1.augmentables.polys",
     None),
    ("_interpolate_point_pair", "imgaugv1.augmentables.polys", None),
    ("_interpolate_points", "imgaugv1.augmentables.polys", None),
    ("_interpolate_points_by_max_distance", "imgaugv1.augmentables.polys", None),
    ("pad", "imgaugv1.augmenters.size", None),
    ("pad_to_aspect_ratio", "imgaugv1.augmenters.size", None),
    ("pad_to_multiples_of", "imgaugv1.augmenters.size", None),
    ("compute_paddings_for_aspect_ratio", "imgaugv1.augmenters.size",
     "compute_paddings_to_reach_aspect_ratio"),
    ("compute_paddings_to_reach_multiples_of", "imgaugv1.augmenters.size", None),
    ("compute_paddings_to_reach_exponents_of", "imgaugv1.augmenters.size", None),
    ("quokka", "imgaugv1.data", None),
    ("quokka_square", "imgaugv1.data", None),
    ("quokka_heatmap", "imgaugv1.data", None),
    ("quokka_segmentation_map", "imgaugv1.data", None),
    ("quokka_keypoints", "imgaugv1.data", None),
    ("quokka_bounding_boxes", "imgaugv1.data", None),
    ("quokka_polygons", "imgaugv1.data", None),
]
