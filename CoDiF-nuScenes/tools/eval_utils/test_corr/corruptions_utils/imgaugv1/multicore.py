...
def _get_context_method():
    ...
    if "NixOS" in platform.version():
        ...
        if get_context_unsupported:
            ia.warn("Detected usage of imgaugv1.multicore in python <=3.4 "
...
class Pool(object):
    ...
    Parameters
    ----------
    augseq : imgaugv1.augmenters.meta.Augmenter
    ...
    def map_batches(self, batches, chunksize=None):
        ...
        Parameters
        ----------
        batches : list of imgaugv1.augmentables.batches.Batch
        ...
        Returns
        -------
        list of imgaugv1.augmentables.batches.Batch
            Augmented batches.
    ...
    def imap_batches(self, batches, chunksize=1, output_buffer_size=None):
        ...
        Parameters
        ----------
        batches : generator of imgaugv1.augmentables.batches.Batch
    ...
    def join(self):
        ...
        :func:`~imgaugv1.multicore.Pool.close` or
        :func:`~imgaugv1.multicore.Pool.terminate`.
...
def _Pool_worker(batch_idx, batch):
    ...
    assert isinstance(batch, (UnnormalizedBatch, Batch)), (
        ...
        "`imgaugv1.augmentables.batches.UnnormalizedBatch` or "
        "`imgaugv1.augmentables.batches.Batch`. Got type %s instead." % (
            type(batch)
        ))
...
class BatchLoader(object):
    ...
    Deprecated. Use ``imgaugv1.multicore.Pool`` instead.

    Loaded batches can be accesses using :attr:`imgaugv1.BatchLoader.queue`.
    ...
    @ia.deprecated(alt_func="imgaugv1.multicore.Pool")
        ...
        try:
            ...
            for batch in gen:
                assert isinstance(batch, Batch), (
                    ...
                    "be of class imgaugv1.Batch, got %s." % (
                        type(batch),))
