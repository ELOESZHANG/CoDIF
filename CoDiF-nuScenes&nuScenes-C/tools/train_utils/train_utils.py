...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
                    ...
                    total_epochs=None, ckpt_save_dir=None, ckpt_save_time_interval=2000, show_gpu_stat=False, use_amp=False, accumulation_steps=1, balance=False):
    ...
    # print('abcabc', model.module.model_cfg.FUSER)
    Train_Mode = model.module.model_cfg.FUSER['TRAIN_MODE']

    if Train_Mode == 'difftrain':
        for name, param in model.named_parameters():
            param.requires_grad = False
             ##### fuser.attentionb
            if 'fuser.attentionb' in name or 'BBBbb' in name:  # 根据层的名称选择需要冻结的层
                param.requires_grad = True
                print(f"解冻层: {name}")
    elif Train_Mode == 'finetune':
        for name, param in model.named_parameters():
            if not param.is_floating_point():
                print(f"跳过非 float 参数: {name} (dtype={param.dtype})")
                continue
            param.requires_grad = True
             ##### fuser.attentionb
            if 'fuser.attentionb' in name or 'BBBbb' in name:  # 根据层的名称选择需要冻结的层
