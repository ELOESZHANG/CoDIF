...
def eval_one_epoch(cfg, args, model, dataloader, epoch_id, logger, dist_test=False, result_dir=None):
    ...
    use_load_anno = dataloader.dataset.dataset_cfg.get('USE_LOAD_ANNO', False)
    use_debug = dataloader.dataset.dataset_cfg.get('USE_DEBUG', False)
    ...
    if cfg.LOCAL_RANK == 0:
        progress_bar.close()
    if save_results and not use_load_anno:
        for key in save_path_dict.keys():
            with open(save_path_dict[key], 'wb') as f:
                pickle.dump(save_data_dict[key], f)
    if dist_test:
