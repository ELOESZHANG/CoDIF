...
class MambaFusion(nn.Module):
    def __init__(self, model_cfg, use_map=False, **kwargs):
        ...
        if self.use_vmamba_pretrain and not self.use_prebackbone:
            ...
            if self.use_multi_scalev_down:
                depths = (2, 2, 2)
                out_indices += [2]
                self.vssm_multi_scalev_down_block = nn.Conv2d(in_channels=512, out_channels=128, kernel_size=1,
                                                              stride=1, padding=0)
            args = {
                'norm_layer': 'ln2d',
                'patch_size': 4,
        ...
        for stage_id in range(stage_num):
            ...
            for i in range(num_blocks_this_stage):
                if (self.use_mamba_inter and i in self.inter_block) \
                    ...
                    if (
                            self.use_multi_scalev or self.use_multi_scalev_down or self.use_multi_scale) and i != self.lidar2image_end - 1:
                    ...
                    for j in range(self.dsb_len):
                        ...
                        if self.use_multi_scalev:
                            ...
                            dsb.append(LocalMamba(dim=128, depth=2, down_scales=self.inter2_down_scales,
                                                  window_shape=self.inter2_win_shape, group_size=self.inter2_win_size,
                                                  direction=['x', 'y'], shift=True,
                                                  operator=EasyDict({'NAME': 'Mamba',
                                                                     'CFG': {'d_state': 16, 'd_conv': 4, 'expand': 2,
                                                                             'drop_path': 0.2}}), layer_id=0,
                                                  n_layer=34, use_expand=self.inter2_use_expand))
                            ...
                            # dsb.append(WinMamba_Block(dim=128, depth=2, down_scales=[[2, 2, 1], [2, 2, 1]], window_shape=[13, 13, 1], group_size=256, direction=['x', 'y'], shift=True,
                            #                    operator=EasyDict({'NAME': 'Mamba', 'CFG': {'d_state': 16, 'd_conv': 4, 'expand': 2, 'drop_path': 0.2}}),layer_id=0, n_layer=34))
                            dsb.append(LocalMamba(dim=128, depth=2, down_scales=[[2, 2, 1], [2, 2, 1]],
                                                  window_shape=self.inter2_win_shape, group_size=self.inter2_win_size,
                                                  direction=['x', 'y'], shift=True,
                                                  operator=EasyDict({'NAME': 'Mamba',
                                                                     'CFG': {'d_state': 16, 'd_conv': 4, 'expand': 2,
                            ...
                            if self.mixed_version == 0:
                                ...
                                dsb.append(LocalMamba(dim=128, depth=2, down_scales=self.inter2_down_scales,
                                                      ...
                                                      shift=self.use_shift,
                                                      operator=EasyDict(self.localmamba_info), layer_id=0, n_layer=34,
                                                      win_version=self.win_version, use_expand=self.inter2_use_expand,
                                                      use_fixed_mapping=self.use_fixed_mapping,
                                                      use_inverse=self.use_inverse,
                                                      use_checkpoint=self.use_checkpoint_local))
                                ...
                                dsb.append(GlobalMamba(128, ssm_cfg=self.ssm_cfg, norm_epsilon=1e-05, rms_norm=True,
                                                       ...
                                                       down_resolution=self.use_down_scale_inter2,
                                                       residual_in_fp32=True, fused_add_norm=True,
                                                       device='cuda', dtype=torch.float32,
                                                       use_checkpoint=self.use_checkpoint_global))
                                ...
                                dsb.append(LocalMamba(dim=128, depth=2, down_scales=self.inter2_down_scales,
                                                      window_shape=self.inter2_win_shape,
                                                      group_size=self.inter2_win_size, direction=['x', 'y'], shift=True,
                                                      operator=EasyDict(self.localmamba_info), layer_id=0, n_layer=34,
                                                      win_version=self.win_version))
                                dsb.append(LocalMamba(dim=128, depth=2, down_scales=self.inter2_down_scales,
                                                      window_shape=self.inter2_win_shape,
                                                      group_size=self.inter2_win_size, direction=['x', 'y'], shift=True,
                                                      operator=EasyDict(self.localmamba_info), layer_id=0, n_layer=34,
                    ...
                    if self.use_winmamba and i == 0:
                        ...
                        if self.mixed_version == 0:
                            block_list.append(
                                nn.ModuleList([
                                    LocalMamba(dim=128, depth=2, down_scales=[[2, 2, 1], [2, 2, 1]],
                                               window_shape=self.inter1_win_shape, group_size=self.inter1_win_size,
                                               direction=['x', 'y'], shift=self.use_shift,
                                               operator=EasyDict(self.localmamba_info), layer_id=0, n_layer=34,
                                               use_checkpoint=self.use_checkpoint_local),
                            ...
                            block_list.append(
                                nn.ModuleList([
                                    LocalMamba(dim=128, depth=2, down_scales=[[2, 2, 1], [2, 2, 1]],
                                               ...
                                               operator=EasyDict({'NAME': 'Mamba',
                                                                  'CFG': {'d_state': 16, 'd_conv': 4, 'expand': 2,
                                                                          'drop_path': 0.2}}), layer_id=0, n_layer=34),
                                    LocalMamba(dim=128, depth=2, down_scales=[[2, 2, 1], [2, 2, 1]],
                                               window_shape=self.inter1_win_shape, group_size=self.inter1_win_size,
                                               direction=['x', 'y'], shift=True,
                                               operator=EasyDict({'NAME': 'Mamba',
                            ...
                            block_list.append(
                                nn.ModuleList([
                                    GlobalMamba(128, ssm_cfg=None, norm_epsilon=1e-05, rms_norm=True,
                                                down_kernel_size=[3, 3], down_stride=[1, 2], num_down=[0, 1],
                                                norm_fn=partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01),
                                                indice_key='stem0_layer0', sparse_shape=[1, 360, 360],
                                                hilbert_config=self.hilbert_config,
                                                downsample_ori='curve_template_rank9',
                                                downsample_lvl='curve_template_rank8',
                                                down_resolution=True, residual_in_fp32=True, fused_add_norm=True,
            ...
            if self.use_mamba_inter2:
                ...
                if self.use_multi_scale:
                    ...
                    for i in range(len(lidar2image_coords_bzyx_list)):
                        lidar2image_coords_bzyx_list[i][:, 0] = lidar2image_coords_bzyx_list[i][:,
                                                                0] * lidar2image_view_num + \
                                                                lidar2image_coords_bzyx_list[i][:, 1]
            ...
            for i in range(len(block_layers)):  # 4 (1 2 1)
                ...
                if self.image2lidar_on and i >= self.image2lidar_start and i < self.image2lidar_end:  # i == 3 模态间注意力机制
                    if self.use_mamba_inter:
                        ...
                        if self.use_mixed:
                            output, _ = block[0](output, image2lidar_coor, batch_dict['batch_size'], self.shape_inter,
                                                 self.curve_template, self.hilbert_spatial_size, self.pos_embed_inter,
                                                 0, False)
                            output = block[1](output, image2lidar_inds_list[stage_id], image2lidar_masks_list[stage_id],
                                              multi_pos_embed_list[stage_id][i],
                                              block_id=block_id, voxel_num=voxel_num,
                                              using_checkpoint=block_id in self.checkpoint_blocks)

                        else:
                            if self.use_checkpoint_inter:
                                output, _ = checkpoint(block, output, image2lidar_coor, batch_dict['batch_size'],
                                                       self.shape_inter,
                                                       self.curve_template, self.hilbert_spatial_size,
                                                       self.pos_embed_inter, 0, False)
                            else:
                                output, _ = block(output, image2lidar_coor, batch_dict['batch_size'], self.shape_inter,
                                                  self.curve_template, self.hilbert_spatial_size, self.pos_embed_inter,
                                                  0, False)
                    else:
                        output = block(output, image2lidar_inds_list[stage_id], image2lidar_masks_list[stage_id],
                                       multi_pos_embed_list[stage_id][i],
                                       block_id=block_id, voxel_num=voxel_num,
                                       using_checkpoint=block_id in self.checkpoint_blocks)
                    ...
                    if self.use_mamba_inter2:
                        if self.use_mixed:
                            ...
                            if i == self.lidar2image_start:
                                ...
                                if self.use_multi_scalev_down:
                                    lidar2image_coor = torch.cat(
                                        [lidar2image_coor, batch_dict['multi_scalev_down_features_coords']], dim=0)
                                    output_extended = torch.cat(
                                        [output_extended, batch_dict['multi_scalev_down_features']], dim=0)
                                if self.use_multi_scale:
                                    use_conv_list = ['x_conv3']
                                    multi_scale_3d_features = batch_dict['multi_scale_3d_features']
                                    extends = torch.cat(
                                        [multi_scale_3d_features[conv_name].features for conv_name in use_conv_list],
                                        dim=1)
                                    lidar2image_coor = torch.cat(
                                        [lidar2image_coor, torch.cat(lidar2image_coords_bzyx_list)], dim=0)
                            ...
                            output_extended, output_sparse = self.forward_stage2(block, output_sparse, lidar2image_coor,
                                                                                 batch_dict, i)
                            ...
                            if i == self.lidar2image_start:
                                ...
                                lidar2image_coor = torch.cat(
                                    [lidar2image_coor, batch_dict['multi_scalev_features_coords']], dim=0)
                                ori_len = output.shape[0]
                            ...
                            if self.use_profiler:
                                with torch.profiler.record_function("block_{}".format(i)):
                                    output_extended, output_sparse = self.forward_stage2(block, output_sparse,
                                                                                         lidar2image_coor, batch_dict,
                                                                                         i)
                            else:
                                output_extended, output_sparse = self.forward_stage2(block, output_sparse,
                                                                                     lidar2image_coor, batch_dict, i)
                            ...
                            if i == self.lidar2image_start:
                                ...
                                multi_scale_3d_features = batch_dict['multi_scale_3d_features']
                                extends = torch.cat(
                                    [multi_scale_3d_features[conv_name].features for conv_name in use_conv_list], dim=1)
                                lidar2image_coor = torch.cat(
                                    [lidar2image_coor, torch.cat(lidar2image_coords_bzyx_list)], dim=0)
                            ...
                            if i == self.lidar2image_start:
                                if self.mask_out_img:
                                    ...
                                    selected_indices = (lidar2image_coor[:, 2] >= 88) & (
                                                lidar2image_coor[:, 2] < 176) & (lidar2image_coor[:, 3] >= 32) & (
                                                                   lidar2image_coor[:, 3] < 64)
                            ...
                            if self.use_fixed_mapping:
                                ...
                                output, output_sparse, fixed_mapping_list = self.forward_stage2(block, output_sparse,
                                                                                                lidar2image_coor,
                                                                                                batch_dict, i,
                                                                                                fixed_mapping_list)
                            else:
                        ...
                        output = block(output, lidar2image_inds_list[stage_id], lidar2image_masks_list[stage_id],
                                       multi_pos_embed_list[stage_id][i],
                                       block_id=block_id, voxel_num=voxel_num,
                                       using_checkpoint=block_id in self.checkpoint_blocks)
                    if self.use_vmamba and self.use_in_mid:
                    ...
                    if self.use_winmamba:
                        ...
                        output = block(output, multi_set_voxel_inds_list[stage_id],
                                       multi_set_voxel_masks_list[stage_id], multi_pos_embed_list[stage_id][i],
                                       block_id=block_id, voxel_num=voxel_num,
                                       using_checkpoint=block_id in self.checkpoint_blocks)
                ...
                if i in self.out_indices:  # []
                    if i == self.out_indices[-1] and self.use_vmamba and self.use_after:
                        ...
                        if self.use_conv:
                            ...
                            image_part = torch.cat(
                                [output[voxel_num:], output_new_self[voxel_num:], output_new_cross[voxel_num:]],
                                dim=-1).contiguous()
    ...
    def _input_preprocess(self, batch_dict):
        ...
        if self.use_prebackbone:
            ...
            if self.use_multi_scalev:
                ...
                batch_dict['multi_scalev_features'] = batch_dict['multi_scalev_features'].permute(0, 2, 3,
                                                                                                  1).contiguous().view(
                    -1, 128)
        ...
        if self.use_cross_img:
            ...
            with torch.no_grad():
                if self.use_vmamba_pretrain:
                    ...
                    sweeps_imgs = sweeps_imgs.permute(0, 2, 3, 1).contiguous().view(B * N, T, -1,
                                                                                    128)  # [6, 2, 2816, 128]
                else:
        ...
        if self.patch_coords is not None and ((self.patch_coords[:, 0].max().int().item() + 1) == B * N):
            ...
            if self.use_multi_scalev:
                ...
                new_patch_x, new_patch_y = torch.meshgrid(
                    torch.arange(new_hw_shape[0], device=imgs.device),
                    torch.arange(new_hw_shape[1], device=imgs.device))
        ...
        patch_set_voxel_inds_list = [
            [patch_info[f'set_voxel_inds_stage{s}_shift{i}']  # 每个set里voxel按照xy排序后的index 每个元素 [2, num_of_set, 90]
             for i in range(self.num_shifts[s])] for s in range(len(self.set_info))]
        patch_set_voxel_masks_list = [
            [patch_info[f'set_voxel_mask_stage{s}_shift{i}']  # 重复voxel的mask 每个元素 [2, num_of_set, 90]
             for i in range(self.num_shifts[s])] for s in range(len(self.set_info))]
        patch_pos_embed_list = [
            [[patch_info[f'pos_embed_stage{s}_block{b}_shift{i}']  # 每个set里每个block的pos_embed 每个元素 [num_of_voxel, 128]
              for i in range(self.num_shifts[s])] for b in range(self.image_pos_num)] for s in
            range(len(self.set_info))]
        ...
        pos_embed_list = [[[voxel_info[f'pos_embed_stage{s}_block{b}_shift{i}']
                            for i in range(self.num_shifts[s])] for b in range(self.lidar_pos_num)] for s in
                          range(len(self.set_info))]
        ...
        multi_set_voxel_inds_list = [
            [torch.cat([set_voxel_inds_list[s][i], patch_set_voxel_inds_list[s][i] + voxel_num], dim=1)
             for i in range(self.num_shifts[s])] for s in
            range(len(self.set_info))]  # 每个set里voxel按照xy排序后的index 每个元素 [2, num_of_set(voxel)+num_of_set(patch), 90]
        multi_set_voxel_masks_list = [[torch.cat([set_voxel_masks_list[s][i], patch_set_voxel_masks_list[s][i]], dim=1)
                                       for i in range(self.num_shifts[s])] for s in range(
            len(self.set_info))]  # 重复voxel的mask 每个元素 [2, num_of_set(voxel)+num_of_set(patch), 90]
            ...
            for b in range(self.set_info[s][1]):  # 4
                ...
                for i in range(self.num_shifts[s]):  # 2
                    if b < self.lidar_pos_num and b < self.image_pos_num:  # ? 好像必然是这个branch
                        if (self.use_mamba_inter2 and b in self.inter2_block) or (
                                self.use_mamba_inter and b in self.inter_block) or (self.use_winmamba and b == 0):
                            shift_pos_embed_list.append([])
                        else:
                            shift_pos_embed_list.append(
                                torch.cat([pos_embed_list[s][b][i], patch_pos_embed_list[s][b][i]],
                                          dim=0))  # [num_of_voxel+num_of_patch, 128]
        ...
        if self.use_cross_img:  # multi_set_voxel_inds_list[0][1][:, set_voxel_inds_list[0][0].shape[1]:,: ] - batch_dict['voxel_num']
            return multi_feat, voxel_info, patch_info, multi_set_voxel_inds_list, multi_set_voxel_masks_list, multi_pos_embed_list, sweeps_imgs, [
                set_voxel_inds_list[0][i].shape[1] for i in range(self.num_shifts[0])]
        else:
    ...
    def _image2lidar_preprocess(self, batch_dict, multi_feat, multi_pos_embed_list):
        ...
        if self.use_mamba_inter:
            if self.use_mixed:
                return image2lidar_inds_list, image2lidar_masks_list, multi_pos_embed_list, image2lidar_batch_dict[
                    'voxel_coords']
            else:
                return None, None, multi_pos_embed_list, torch.cat(
                    [batch_dict['voxel_coords'], image2lidar_coords_bzyx], dim=0)
        else:
    ...
    def _lidar2image_preprocess(self, batch_dict, multi_feat, multi_pos_embed_list):
        ...
        lidar2image_coords_zyx, lidar2image_coords_bzyx_list = self.map_lidar2image_layer(batch_dict,
                                                                                          self.use_multi_scale)  # [num_of_voxel, 3] view_idx, x, y 点云投影到图像上的坐标
        ...
        if not (self.use_mamba_inter2 and not self.use_mixed):
            ...
            lidar2image_batch_dict['voxel_coords'] = torch.cat(
                [lidar2image_coords_bzyx, multiview_coords],
                dim=0)  # torch.Size([num_of_voxel + num_of_patch, 4]) batch_id view_id y x
            ...
            lidar2image_pos_embed_list = [[[lidar2image_info[f'pos_embed_stage{s}_block{b}_shift{i}']
                                            for i in range(self.num_shifts[s])] for b in
                                           range(self.lidar2image_pos_num)] for s in range(len(self.set_info))]
        ...
        if self.use_mamba_inter2:
            if self.use_mixed:
                return lidar2image_inds_list, lidar2image_masks_list, multi_pos_embed_list, lidar2image_batch_dict[
                    'voxel_coords']
            elif self.use_multi_scale:
                return None, None, multi_pos_embed_list, torch.cat([lidar2image_coords_bzyx, multiview_coords],
                                                                   dim=0), lidar2image_coords_bzyx_list
            else:
