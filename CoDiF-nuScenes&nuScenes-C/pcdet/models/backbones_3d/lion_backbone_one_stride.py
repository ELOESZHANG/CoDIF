...
class LION3DBackboneOneStride(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        for d in range(len(depths)):
            assert len(layer_down_scales[d]) == depths[d]
        assert len(self.layer_dim) == len(depths)

        layers_per_block = 2 * 2  # encoder/decoder × directions
        layers_per_depth = lambda d: layers_per_block * d
        for i in range(num_layers):
            layer_id = sum(layers_per_depth(depths[j]) for j in range(i))
            setattr(self, f'linear_{i+1}',
                LIONBlock(self.layer_dim[i], depths[i], layer_down_scales[i],
                          self.window_shape[i], self.group_size[i], direction,
                          shift=shift, operator=self.linear_operator,
                          layer_id=layer_id, n_layer=self.n_layer))
            in_dim = self.layer_dim[i]
            out_dim = self.layer_dim[i + 1] if i < num_layers - 1 else self.layer_dim[i]
            setattr(self, f'dow{i+1}',
                PatchMerging3D(in_dim, out_dim, down_scale=[1, 1, 2],
                               norm_layer=norm_fn, diffusion=diffusion, diff_scale=diff_scale,
                               return_abs_coords=self.use_height_fidelity))

        last_layer_id = sum(layers_per_depth(depths[i]) for i in range(num_layers))
        self.linear_out = LIONLayer(self.layer_dim[-1], 1, [13, 13, 2], 256, direction=['x', 'y'], shift=shift,
                                     operator=self.linear_operator, layer_id=last_layer_id, n_layer=self.n_layer)
        self.use_dow5 = model_cfg.get('USE_DOW5', False)
        self.dow5_diff = model_cfg.get('DOW5_DIFF', True)
        if self.use_dow5:
            initial_z = self.sparse_shape[0]  # e.g. 32
            dow5_z_scale = initial_z // (2 ** num_layers)  # ensures final z=1
            self.dow5 = PatchMerging3D(self.layer_dim[-1], self.layer_dim[-1],
                                        down_scale=[1, 1, dow5_z_scale],
                                        norm_layer=norm_fn, diffusion=self.dow5_diff and diffusion, diff_scale=diff_scale,
                                        return_abs_coords=self.use_height_fidelity)

        self.num_point_features = dim

        self.backbone_channels = {
            f'x_conv{i+1}': self.layer_dim[i] for i in range(num_layers)
        }
    ...
    def forward(self, batch_dict):
        ...
        if self.use_height_fidelity:
            ...
            for i in range(num_layers):
                x = getattr(self, f'linear_{i+1}')(x)
                x, _, ori_coords_height = getattr(self, f'dow{i+1}')(x, ori_coords_height=ori_coords_height)
                conv_outputs[f'x_conv{i+1}'] = x
                batch_dict[f'ori_coords_height_coords{i+1}'] = ori_coords_height

            x = self.linear_out(x)
            if self.use_dow5:
                x, _, ori_coords_height = self.dow5(x, ori_coords_height=ori_coords_height)
            batch_dict['ori_coords_height'] = ori_coords_height
        else:
            for i in range(num_layers):
                x = getattr(self, f'linear_{i+1}')(x)
                x, _ = getattr(self, f'dow{i+1}')(x)
                conv_outputs[f'x_conv{i+1}'] = x
        ...
        batch_dict.update({
            'multi_scale_3d_features': conv_outputs,
            'multi_scale_3d_strides': {
                f'x_conv{i+1}': torch.tensor([1, 1, 2 * (2 ** i)], device=x.features.device).float()
                for i in range(num_layers)
            }
        })
