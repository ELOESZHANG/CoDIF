...
def depth_to_lidar(idx_filename, split, dense_depth_dir, pseudo_depth_dir):
    dataset = kitti_object('data/kitti_pseudo', split)
    ...
    for data_idx in tqdm.tqdm(data_idx_list):
        ...
        i = h - th
        j = int(round((w - tw) / 2.))
        img_depth = np.zeros((h, w),dtype=float)
...
depth_to_lidar( \
    'data/kitti_pseudo/ImageSets/trainval.txt',
    'training',
    'data/kitti_pseudo/training/depth_dense_twise',
    'data/kitti_pseudo/training/depth_pseudo_rgbseguv_twise'
)

print('---------------Start to generate testing pseudo point clouds---------------')
depth_to_lidar( \
    'data/kitti_pseudo/ImageSets/test.txt',
    'testing',
    'data/kitti_pseudo/testing/depth_dense_twise',
    'data/kitti_pseudo/testing/depth_pseudo_rgbseguv_twise'
)