...
#from opencood.models.sub_modules.torch_transformation_utils import warp_affine as warp_affine_simple
...
class Unet(nn.Module):
    ...
    def forward(self, x, time, x_self_cond = None, condition = None, given_cond = False):
        ...
        #x torch.Size([1, 16, 104, 88])
        #condition torch.Size([1, 16, 104, 88])
        if given_cond:  
...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance_condition(self, x, time, time_next, condition):
        ...
        log_snr = self.log_snr(time)
        log_snr_next = self.log_snr(time_next)
        log_snr = log_snr.squeeze() if isinstance(log_snr, torch.Tensor) else log_snr
        log_snr_next = log_snr_next.squeeze() if isinstance(log_snr_next, torch.Tensor) else log_snr_next
        ...
        # batch_log_snr = repeat(log_snr, ' -> b', b = x.shape[0])
        batch_log_snr = torch.full((x.shape[0],), log_snr, device=x.device)
    ...
    @torch.no_grad()
    def ddim_sample_condition(self, batch_size = 16, condition = None, sample_stepss=None):
        if sample_stepss is None:
            sample_stepss=self.num_sample_steps

        return self.ddim_sample_loop(shape=(batch_size, self.channels, self.image_size[0], self.image_size[1]), condition = condition, sample_stepss=sample_stepss)
    ...
    def ddim_sample(
        ...
        eps = self._predict_eps_from_xstart(x, t, out[0])
        ...
        return {"sample": sample, "pred_xstart": out[0]}
    ...
    def ddim_sample_loop_progressive(
        ...
        indices = list(range(0, self.num_timesteps, self.num_timesteps//sample_stepss))[::-1]
        # print(indices)
        ...
        for i in indices:
            ...
            t_next = th.tensor([i + self.num_timesteps//sample_stepss] * shape[0], device=self.device)
            # print(t_next)
    ...
    def p_losses_with_condition(self, x_start, times, condition = None,noise = None):
        ...
        # loss = F.mse_loss(model_out[:,:,2:102,:], target[:,:,2:102,:], reduction = 'none')
        loss = F.mse_loss(model_out[:, :, :, :], target[:, :, :, :], reduction='none')
    ...
    def forward(self, img, condition, *args, **kwargs):
        ...
        # 计算每个batch的条件
        current_condition = condition[:, 0].reshape(B, *condition.shape[2:]) # shape [B, 8, 104, 88]
...
class DifussionFusion(nn.Module):
    def __init__(self, args):
        ...
        if 'train_mode' in args.keys():
            ...
            self.steps = args['train_steps']
            self.sample_stepss=args['sample_stepss']
        else:
            self.train_mode = 'finetune'
            self.steps = 500
            self.sample_stepss=20
        ...
        self.diffusion = GaussianDiffusion(
            ...
            image_size= (64, 128),
            channels=self.channels,
            num_sample_steps=self.steps) # 100 when training
    ...
    def sample(self, x, heter_feature_2d):
        """
        Fusion forwarding.
        ...
        # 2. Diffusion
        regroup_feature = heter_feature_2d
        ...
        sampled_feature = self.diffusion.ddim_sample_condition(
            ...
            sample_stepss=self.sample_stepss
        )

        # sampled_feature = sampled_feature[:,:,2:102,:]
        sampled_feature = sampled_feature[:, :, :, :]

        return sampled_feature
    
    def forward(self, x, heter_feature_2d):
        """
        Fusion forwarding.
        ...
        regroup_feature = heter_feature_2d.unsqueeze(1)  # [B, L, C, H, W]
        #将H W从 100 88 padding到 104 88  #dair
        # regroup_feature = F.pad(regroup_feature, (0, 0, 2, 2), "constant", 0)
        # x = F.pad(x, (0, 0, 2, 2), "constant", 0)
        print("regroup_feature:", regroup_feature.shape)
        print("x:", x.shape)
...
if __name__ == '__main__':
    ...
    # Test the diffusion fusion model
    ...
    model = DifussionFusion(config).to(device)

    # Create test inputs
    batch_size = 1
    channels = 16
    height, width = 64, 128  # Original size

    # Create test tensors and move to CUDA
    x = torch.randn(batch_size, channels, height, width).to(device)  # Fused feature
    heter_feature_2d = torch.randn(batch_size, channels, height * 1, width * 1).to(
        device)  # Heterogeneous features

    # Test forward pass
    loss = model(x, heter_feature_2d)
    print(f"Forward pass loss: {loss.item()}")

    # Test sampling
    with torch.no_grad():
        sampled_feature = model.sample(x, heter_feature_2d)
        print(f"Sampled feature shape: {sampled_feature.shape}")