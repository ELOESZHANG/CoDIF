...
class SparseConvolution(SparseConvolutionBase, SparseModule):
    ...
    def forward(self, input: SparseConvTensor):
        ...
        # calculate flops
        flops = (pair_fwd > -1).sum() * features.shape[1] * out_features.shape[1]
        if self.bias is not None:
            flops += out_features.shape[0]
    ...
    def count_your_model(m, x, y):
        m.total_ops += torch.DoubleTensor([int(y.flops)])
        m.total_acts += y.features.shape[0] * y.features.shape[1]
