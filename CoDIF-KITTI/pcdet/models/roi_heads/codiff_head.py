...
from .codiff_part import ColorEh, Fusion3



class CODIFFhead(RoIHeadTemplate):
    def __init__(self, input_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, **kwargs):
        ...
        # RoI Point Pool
        # self.colorbase_layer1 = Baseline_color()
        self.colorbase_layer1 = ColorEh()
        ...
        channel1 = self.model_cfg.ROI_AWARE_POOL.NUM_FEATURES_RAW
        channel2 = self.model_cfg.ROI_AWARE_POOL.NUM_FEATURES

        ###NSC & Baseline
        self.fc1 = nn.Sequential(nn.Linear(channel1, channel2),
                                 nn.SELU(),
                                 )

        self.fc2 = nn.Sequential(nn.Linear(216 * 8, 216),
                                 nn.SELU(),
                                 )
        self.sig1 = nn.Sigmoid()
        self.para1 = nn.Parameter(torch.rand(1))
        self.para2 = nn.Parameter(torch.rand(1))

        self.fusion_layer = Fusion3(c_out, channel2, self.model_cfg.ATTENTION_FC[0],self.model_cfg.Mamba_CFG,self.model_cfg.CoDiff_CFG, self.model_cfg.LOSS_CONFIG.Train_Mode)
        shared_pre_channel = GRID_SIZE * GRID_SIZE * GRID_SIZE * c_out * 2  #c_out*2 4*2
        shared_fc_list = []
        ...
        if self.training:
            ...
            pre_channel_pseudo = GRID_SIZE * GRID_SIZE * GRID_SIZE * channel2 #channel2 4
            ...
            self.reg_pred_layer_pseudo = nn.Linear(reg_pre_channel_pseudo,
                                                   self.model_cfg.AUXILIARY_CODE_SIZE * self.num_class, bias=True)

        self.init_weights()
        self.reg_loss_func_pseudo=loss_utils.WeightedSmoothL1Loss(code_weights=self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS_PSEUDO['code_weights'])
    ...
    def roicrop3d_gpu(self, batch_dict, pool_extra_width):
        ...
        enlarged_rois = box_utils.enlarge_box3d(rois.view(-1, 7).clone().detach(), pool_extra_width).view(batch_size,
                                                                                                          -1, 7)
        ...
        image[total_pts_features[:, 7].long(), total_pts_features[:, 6].long()] = global_index.to(
            device=total_pts_features.device)
    ...
    def forward(self, batch_dict):
        ...
        frame_idss = batch_dict['frame_id']
        ...
        points_features_expand = self.colorbase_layer1(points_features, points_neighbor,frame_idss,points_coords_src)
        # print('points_features:',points_features.shape)
        # print('points_neighbor:', points_neighbor.shape)
        # print('points_coords_src:', points_coords_src.shape)
        # print('points_features_expand:', points_features_expand.shape)


        batch_dict['points_pseudo_features'] = torch.cat([points_batch, points_coords_src, points_features_expand[1:]],
                                                         dim=-1)
        # print("batch_dict['points_pseudo_features']", batch_dict['points_pseudo_features'].shape)

        # RoI Voxel Pool
        pooled_pseudo_features = self.roiaware_pool(batch_dict)

        #baseline-No-NSC
        # x_pseudo = pooled_pseudo_features.reshape(pooled_pseudo_features.shape[0], -1, pooled_pseudo_features.shape[4])
        # x_pseudo= self.fc1(x_pseudo)
        # x_pseudo = x_pseudo.permute(0, 2, 1)
        ...
        if self.training:
            ...
            rb = 0.1 #0.1
        x_pseudo = pooled_pseudo_features.reshape(pooled_pseudo_features.shape[0], -1, pooled_pseudo_features.shape[4])  # 128,216*8,102
        x_pseudo_heatmap = torch.max(x_pseudo, dim=1, keepdim=True)[0]  # 128,1,102
        x_pseudo_heatmap = self.fc1(x_pseudo_heatmap)  # 128,1,128
        x_pseudo_heatmap = self.sig1(x_pseudo_heatmap) + ra/(self.para1[0]+rb)  # 128,1,128
        x_pseudo = self.fc1(x_pseudo)  # 128，216*8,128
        x_pseudo = x_pseudo * x_pseudo_heatmap * (self.para1[0]+rb)

        x_pseudo=x_pseudo.permute(0, 2, 1)
        x_pseudo_heatmap = torch.max(x_pseudo, dim=1, keepdim=True)[0]  # 128,1,216*8
        x_pseudo_heatmap = self.fc2(x_pseudo_heatmap) # 128,1,216
        x_pseudo_heatmap = self.sig1(x_pseudo_heatmap) + ra/(self.para2[0]+rb) # 128,1,216
        x_pseudo = self.fc2(x_pseudo) * x_pseudo_heatmap * (self.para2[0]+rb) #128,128,216
        frame_idss = batch_dict['frame_id']

        train_mode = self.model_cfg.LOSS_CONFIG.Train_Mode

        loss_diff, fusion_features = self.fusion_layer(x_valid, x_pseudo,frame_idss,train_mode)
        ...
        if not self.training:
            ...
            # batch_cls_preds_p, batch_box_preds_p = self.generate_predicted_boxes(
            #     batch_size=batch_dict['batch_size'], rois=batch_dict['rois'], cls_preds=rcnn_cls_pseudo, box_preds=rcnn_reg_pseudo
            # )
            # batch_cls_preds = torch.cat([batch_cls_preds, batch_cls_preds_p], dim=1)
            # batch_box_preds = torch.cat([batch_box_preds, batch_box_preds_p], dim=1)
            ...
            targets_dict['loss_diff'] = loss_diff

            # print('loss_diff', loss_diff.item())
    ...
    def get_loss(self, tb_dict=None):
        ...
        loss_diff=self.forward_ret_dict.get('loss_diff')
        ...
        return rcnn_loss , tb_dict, loss_diff
    ...
    def get_box_reg_layer_loss(self, forward_ret_dict):
        ...
        if True:
            ...
            elif loss_cfgs.REG_LOSS == 'coiou':
                iou_loss, iou3d = oriented_iou_loss.cal_coiou_3d(dt_boxes.unsqueeze(0), gt_boxes.unsqueeze(0))
    ...
    def get_box_reg_layer_loss_pseudo(self, forward_ret_dict):
        ...
        if loss_cfgs.REG_LOSS_PSEUDO == 'smooth-l1':
            ...
            # rcnn_reg= rcnn_reg.to(torch.float32)
            # reg_targets_pseudo = reg_targets_pseudo.to(torch.float32)
