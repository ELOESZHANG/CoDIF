...
from pcdet.models.roi_heads.model_util.Diff_JIT import JiT_models
...
class Denoiser(nn.Module):
    ...
    def generate(self, labels):
        ...
        bsz = labels.size(0)
        z = self.noise_scale * torch.randn(bsz, 8, self.img_size, self.img_size, device=device) #3
...
if __name__ == "__main__":
    ...
    x = torch.randn(BS, inc, args.img_size, args.img_size).to(device)
    labels = torch.randint(0, args.class_num, (BS,)).to(device)
    ...
    # Test generation
    try:
        generated = denoiser.generate(labels)
