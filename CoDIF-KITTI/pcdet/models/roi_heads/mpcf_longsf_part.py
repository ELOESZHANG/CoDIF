...
class UrsusInteract(nn.Module):
    ...
    def forward(self,x_img_0,x_lid_0):
        ...
        #img_roi: torch.Size([1,128,128,216])
        #lidar_roi: torch.Size([1,128,128,216])

        # xxxxoa = x_lid.cpu().detach().numpy()

        Hh ,Cl ,Wl =x_lid_0.shape
        # print(x_lid_0.shape)
        # x_img_0 = self.conv_init_img(x_img)
        # x_lid_0 = self.conv_init_lid(x_lid)
        ...
        # batch_dict['ROI_features_0'] = x_lid + x_lid_0
        # batch_dict['ROI_features_img'] =x_img + x_img_0
        # batch_dict['ROI_features_maps'] =maps
...
class UrsusFusion(nn.Module):
    def __init__(self,Mamba_CFG,in_channel) -> None:
        ...
        # self.conv_init_img = nn.Conv2d(64, out_channel, 3, stride=2,padding=1, bias=False)
        # self.conv_init_lid = nn.Conv2d(out_channel, out_channel, 3, stride=2,padding=1, bias=False)
