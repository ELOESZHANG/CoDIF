...
class BaseBEVBackbone(nn.Module):
    ...
    #     self.init_cfg_3d = self.model_cfg.get('INIT_CFG_3D', None)
    #     self.frozen_3d_stages = self.model_cfg.get('FROZEN_3D_STAGES', -1)
    #
    # def train(self, mode=True):
    #     """Convert the model into training mode while keep layers freezed."""
    #     super(LION3DBackbone_Multimodal, self).train(mode)
    #     self._freeze_Layers()
    ...
    #
    #     ###Freeze Every Layer
    #     for i in range(1, self.frozen_3d_stages+1):
    #         linear_ = getattr(self, f'linear_{i}')
    #         # linear_.eval()
    #         for param in linear_.parameters():
    #             param.requires_grad = False
    #
    #     for i in range(1, self.frozen_3d_stages+1):
    #         dow_ = getattr(self, f'dow{i}')
    #         # dow_.eval()
    #         for param in dow_.parameters():
    #             param.requires_grad = False
    #     # if self.frozen_3d_stages > 0:
    #     #     linear_out_=self.linear_out
    #     #     # linear_out.eval()
    #     #     for param in linear_out_.parameters():
    #     #         param.requires_grad = False
    #
    #
    # def init_weights(self):
    #
    #     assert 'checkpoint_3D' in self.init_cfg_3d, f'Only support ' \
    #                                           f'specify `Pretrained3D` in ' \
    #                                           f'`init_cfg_3D` in ' \
    #                                           f'{self.__class__.__name__} '
    #     ckpt = torch.load(self.init_cfg_3d.checkpoint_3D, map_location='cpu')
    #     print('Use 3D checkpoint')
    #     print('load_3d_backbone:', self.init_cfg_3d.checkpoint_3D)
    #     # load state_dict
